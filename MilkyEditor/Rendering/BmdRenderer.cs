/*
    Copyright 2016-2017 shibboleet, StapleButter, miluaces
...
namespace MilkyEditor.Rendering
{
                ...
                foreach (Bmd.Batch.Packet packet in batch.Packets)
                {
                    ...
                    foreach (Bmd.Batch.Packet.Primitive prim in packet.Primitives)
                    {
                        ...
                        for (int i = 0; i < prim.NumIndices; i++)
                        {
                            ...
                            if ((prim.ArrayMask & (1 << 0)) != 0) Vector3.TransformNormal(ref pos, ref mtxtable[prim.PosMatrixIndices[i]], out pos);
                            else Vector3.TransformNormal(ref pos, ref mtxtable[0], out pos);
                            GL.Vertex3(pos);
                        }
